...
func TestAddProduct(t *testing.T) {
	...
	ProductMockInterface := NewMockProductStore(mockCtrl)
...
func TestAddVariant(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:          "success",
			mock:          ProductMockInterface.EXPECT().AddVariant(gomock.Any(), gomock.Any()).Return(1, nil),
			expOut:        1,
			productID:     "1",
			expectedError: nil,
			expVariant: &models.Variant{
	...
	for _, tc := range testCases {
		...
		t.Run("testing create service", func(t *testing.T) {
			output, err := ProductService.AddVariant(k, tc.expVariant)
...
func TestGetVariant(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:          "success case",
			mock:          ProductMockInterface.EXPECT().GetVariant(gomock.Any(), gomock.Any(), gomock.Any()).Return(&models.Variant{}),
			expOut:        &models.Variant{},
			expectedError: nil,
			productID:     1,
		},
	...
	for _, tc := range testCases {
		...
		t.Run("testing create service", func(t *testing.T) {
			output, err := ProductService.GetVariant(k, strconv.Itoa(tc.productID), strconv.Itoa(tc.varID))
